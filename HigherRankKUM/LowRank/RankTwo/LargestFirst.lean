import HigherRankKUM.LowRank.RankTwo.Indexing

namespace HigherRankKUM.RankTwo

/-- The finite fiber of a block label. -/
def fiberFinset {n m : ℕ}
    (block : Fin n → Fin m) (c : Fin m) : Finset (Fin n) :=
  Finset.univ.filter fun j => block j = c

/-- A monotone fiber contains the full interval between any two members. -/
theorem monotone_fiber_interval
    {n m : ℕ} (block : Fin n → Fin m) (hmono : Monotone block)
    {a b j : Fin n} (_hab : a ≤ b) (heq : block a = block b)
    (haj : a ≤ j) (hjb : j ≤ b) :
    block j = block a := by
  apply le_antisymm
...
/-- A monotone fiber containing `a ≤ b` has at least the interval size. -/
theorem fiber_card_ge_interval
    {n m : ℕ} (block : Fin n → Fin m) (hmono : Monotone block)
    {a b : Fin n} (hab : a ≤ b) (heq : block a = block b) :
    b.val - a.val + 1 ≤ (fiberFinset block (block a)).card := by
  have hsub : Finset.Icc a b ⊆ fiberFinset block (block a) := by
    intro j hj
    rw [Finset.mem_Icc] at hj
    simp only [fiberFinset, Finset.mem_filter, Finset.mem_univ, true_and]
    exact monotone_fiber_interval block hmono hab heq hj.1 hj.2
  have hc := Finset.card_le_card hsub
  have hIcc : (Finset.Icc a b).card = b.val - a.val + 1 := by
    simp
    omega
  rw [hIcc] at hc
  exact hc
...
theorem cyclicSucc_val_of_ne_last
    (k : ℕ) (hk : 0 < k) (i : Fin k) (hi : i ≠ lastFin k hk) :
...
/--
The block hypothesis needed by the half-weave argument: contiguous blocks,
each of size at most `k`, with the first block maximal.
-/
structure LargestFirstBlockModel (k m : ℕ) where
  block : Fin (2 * k) → Fin m
  monotone_block : Monotone block
  fiber_card_le : ∀ c, (fiberFinset block c).card ≤ k
  first_fiber_largest :
    ∀ (hk : 0 < k) c,
      (fiberFinset block c).card ≤
        (fiberFinset block (block (firstIndex k (zeroFin k hk)))).card

/-- Paired first- and second-half positions lie in different blocks. -/
theorem largestFirst_first_second_blocks_ne
    {k m : ℕ} (S : LargestFirstBlockModel k m) (i : Fin k) :
    S.block (firstIndex k i) ≠ S.block (secondIndex k i) := by
  intro heq
  have hab : firstIndex k i ≤ secondIndex k i := by
    change i.val ≤ k + i.val
    omega
  have hlower := fiber_card_ge_interval S.block S.monotone_block hab heq
  have hupper := S.fiber_card_le (S.block (firstIndex k i))
  have hlower' :
      k + 1 ≤ (fiberFinset S.block (S.block (firstIndex k i))).card := by
    simpa [firstIndex, secondIndex] using hlower
  omega

/-- A nonfinal bridge from the second half to the next first-half position crosses blocks. -/
theorem largestFirst_second_next_blocks_ne_nonfinal
    {k m : ℕ} (hk : 0 < k) (S : LargestFirstBlockModel k m)
    (i : Fin k) (hi : i ≠ lastFin k hk) :
  ...
  have hsuccval : (cyclicSucc k hk i).val = i.val + 1 :=
  ...
  change
    (k + i.val) - (cyclicSucc k hk i).val + 1 ≤
      (fiberFinset S.block (S.block a)).card at hmid_lower_raw
  rw [hsuccval] at hmid_lower_raw
  have hmid_lower : k ≤ (fiberFinset S.block (S.block a)).card := by omega
  have hmid_upper := S.fiber_card_le (S.block a)
  have hmid_eq : (fiberFinset S.block (S.block a)).card = k := by omega
  have hfirst_lower : k ≤ (fiberFinset S.block (S.block z)).card := by
  ...
  have hfirst_eq : (fiberFinset S.block (S.block z)).card = k := by omega
  have hza : S.block z = S.block a := by
    ...
    have hfirst_sub : fiberFinset S.block (S.block z) ⊆ Finset.Iic p := by
      intro j hj
    ...
    have hsmall : (fiberFinset S.block (S.block z)).card ≤ i.val + 1 := by
  ...
  have hzb_order : z ≤ b := by
    change 0 ≤ k + i.val
    omega
  have hlong := fiber_card_ge_interval S.block S.monotone_block hzb_order hzb
  change
    (k + i.val) - 0 + 1 ≤ (fiberFinset S.block (S.block z)).card at hlong
  have htoo_many : k + 1 ≤ (fiberFinset S.block (S.block z)).card := by omega
  omega

/-- The cyclic wraparound bridge crosses blocks. -/
theorem largestFirst_wraparound_blocks_ne
  ...
  have hab : a ≤ b := by
    ...
    omega
  have heq_ab : S.block a = S.block b := heq_ba.symm
  have hlong := fiber_card_ge_interval S.block S.monotone_block hab heq_ab
  change
    (k + (k - 1)) - 0 + 1 ≤ (fiberFinset S.block (S.block a)).card at hlong
  have htwo : 2 * k ≤ (fiberFinset S.block (S.block a)).card := by omega
...
theorem largestFirst_woven_successor_blocks_ne
  ...
  have hadj := woven_successor_adjacency k hk (Equiv.refl (Fin (2 * k))) p
  ...
  · have hindex :
    ...
    have hsecond' : halfWeaveEquiv k hk (weaveNext k hk p) = secondIndex k i := by
