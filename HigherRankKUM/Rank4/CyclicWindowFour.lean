...
/-- A length-three cyclic window is exactly its three successive entries. -/
theorem cyclicWindow_three_eq
    {E : Set α} {n : ℕ}
    (hn : 0 < n) (σ : Fin n ≃ E) (i : Fin n) :
...
/-- Rank-three cyclic-basis ordering can be checked by the explicit three
successive entries at every start. -/
theorem cyclicBasisOrder_three_iff
    {M : Matroid α} {n : ℕ}
    (hn : 0 < n) (σ : Fin n ≃ M.E) :
    CyclicBasisOrder M 3 hn σ ↔
      ∀ i : Fin n,
        M.IsBase
          ({(σ i : α),
            (σ (cyclicIndex n hn i 1) : α),
  ...
  constructor
  · intro h i
    simpa [cyclicWindow_three_eq] using h i
  · intro h i
